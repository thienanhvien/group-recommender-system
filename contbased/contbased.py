...
product_images = pd.read_csv('Files/Product_image1.csv', index_col=0)
...
def content_based_product(name, n = 10):
    ...
    # Convert the price column to string before merging with top_n_products
    product_data['price'] = product_data['price'].astype(str)

    # Merge product images with top_n_products
    top_n_products = pd.merge(top_n_products, product_images, on='product_name')

    # Return the dataframe
    return top_n_products[['Similarity-Score', 'product_name', 'image', 'price']]
